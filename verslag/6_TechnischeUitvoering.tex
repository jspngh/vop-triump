...
Triump maakt gebruik van App Engine Datastore als backend databank. GAE Datastore verschilt van traditionele relationele databases door zijn specifieke architectuur. De database is namelijk zo ontworpen dat deze automatisch meeschaalt met de toenemende hoeveelheid opgeslagen data. Een tweede verschil met relationele databanken is dat GAE Datastore schemaloos is. Dit werd gezien als het grootste nadeel van de GAE Datastore tegenover een traditionele relationele database. Data wordt in de Datastore opgeslagen als objecten.
Aangezien de database schemaloos is en bijgevolg geen types zal controleren of niet zal aangeven indien een vreemde sleutel niet bestaat ligt de verantwoordelijkheid van de integriteit van de database volledig bij de ontwikkelaar. 
...
\section{Backend}
\subsection{Database}
Zoals reeds aangegeven word de backend database gehost op een GAE instantie. De GAE Datastore is schemaloos wat impliceert dat er geen enkele restrictie met betrekking tot de opbouw van entiteiten wordt opgelegd. Toch is er gekozen voor de database te ontwerpen als een relationele databank aangezien dit toelaat bepaalde operaties zeer efficiënt aan te pakken. Het behoud van de integriteit nu wel volledig bij de ontwikkelaars. 
