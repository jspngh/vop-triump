...
% Voornadelen / nadelen
% Werking
Het technische aspect van Triump bestaat uit twee grote delen. Enerzijds is er de frontend, waarmee de gebruikers rechtstreeks interageren, en anderzijds is er de backend, verborgen voor iedere gebruiker. De frontend bestaat op zijn beurt uit twee delen, nl. een Android applicatie en een webinterface. De backend is ontwikkeld op Google Cloud Endpoints. In onderstaande paragrafen wordt er dieper ingegaan op deze onderdelen. 

\section{Backend}

Figuur \ref{fig:algemene structuur backend} geeft een abstract overzicht van de verschillende onderdelen van de backend en van de interacties tussen de backend en de frontend. De pijlen aangeduid in figuur \ref{fig:algemene structuur backend} worden in de komende secties verklaard.
...
Zoals reeds aangegeven is GAE Datastore schemaloos. Bijgevolg wordt er hierdoor geen enkele restrictie opgelegd aan de databank over de opbouw en de structuur van zijn entiteiten. Toch is de backend databank ontworpen als een relationele databank.
...
De GCM tabel van de backend database houdt voor iedere gebruiker een gcmId bij. Deze ID wordt gebruikt voor de Google Cloud Messaging dienst.
...
Een groot deel van de methodes hebben als doel het ophalen van gegevens uit de database, deze gegevens te mappen op een klasse om vervolgens een object terug te geven als resultaat. 

Een tweede soort API calls wordt gebruikt om entiteiten van de tabellen Group, Event, UserGroup en UserEvent aan te maken of te wijzigen. De belangrijkste methode van dit soort is generateRewards(). De methode generateRewards() wordt vanuit een Cron Job (zie sectie \ref{sec: GCE} Cron jobs) ieder kwartier opgeroepen. Prinicpieel werkt de functie als volgt:
\begin{enumerate}
...
\subsection{Foursquare API~\cite{FS_API_website}}
