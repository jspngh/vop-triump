...
public class FoursquareAPI {
    ...
    public FoursquareVenue getVenueInfo(String venueId)  {
        ...
        if(v.getCount()>0){
            ...
            verified = v.getInt(v.getColumnIndexOrThrow(VenueTable.COLUMN_VERIFIED)) != 0;
...
/*
    ...
    public ArrayList<FoursquareVenue> searchVenues(double latitude, double longitude, int radius, String query)  {
        ...
        String url =API_URL + "/venues/search?ll=" + latitude +","+longitude +"&radius="+radius+"&query="+query+"&oauth_token="+ FSQToken + "&v=" + VERSION+ "&m=" + MODE;
    ...
    //only returns verified venues
    public ArrayList<FoursquareVenue> searchVenuesByName(String q, int limit) throws UnsupportedEncodingException {

        String query = URLEncoder.encode(q, "utf-8");

        String url =API_URL + "/venues/search?query=" + query +"&limit="+limit+"&intent=global&oauth_token="+ FSQToken + "&v=" + VERSION+ "&m=" + MODE;
        //Log.d("FoursquareAPI", url);
        ArrayList<FoursquareVenue> venueList = new ArrayList<>();
